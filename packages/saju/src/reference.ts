import type { BirthInput, FourPillars, TenStars, FiveElements } from "./output-types";
...
export async function getReferenceSajuData(
  ...
  try {
    ...
    const makeOrderData = (await makeOrderResponse.json()) as {
      order3Id?: number;
    };
    ...
    // 외부 forceteller API 응답은 스키마가 고정돼 있지 않아 any 로 둔다(기존 동작 유지).
    const sajuResult = (await freeOrderResponse.json()) as any;
