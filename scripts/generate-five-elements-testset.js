#!/usr/bin/env node
/**
 * 오행 테스트셋 생성 스크립트 (Standalone)
 * Reference API를 사용하여 정확한 오행 데이터를 수집
 */
...
// 오행 정보 추출 함수
function extractFiveElements(apiResult) {
...
async function generateTestset() {
  ...
  for (let i = 0; i < testInputs.length; i++) {
    ...
    try {
      ...
      if (result && result.saju) {
        const fiveElements = extractFiveElements(result);
        testCases.push({ description, input, expected: fiveElements });
        console.log(
          `  ✅ 성공: 목:${fiveElements.wood}, 화:${fiveElements.fire}, 토:${fiveElements.earth}, 금:${fiveElements.metal}, 수:${fiveElements.water}`
        );
      } else {
  ...
  const outputPath = path.join(process.cwd(), 'data', 'five_elements_testset.json');
