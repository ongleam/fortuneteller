...
function normalizeBirthTime(hour: string | null | undefined, minute?: string | null | undefined): string {
...
export async function getReferenceSajuData(
  ...
  try {
    ...
    if (!makeOrderResponse.ok) {
      throw new Error(`주문 생성 실패: ${makeOrderResponse.status} ${makeOrderResponse.statusText}`);
    }
    ...
    if (!freeOrderResponse.ok) {
      throw new Error(`사주 결과 조회 실패: ${freeOrderResponse.status} ${freeOrderResponse.statusText}`);
    }
