...
interface SajuCalculator {
  calculateSaju(solarDate: Date, birthTime: string, gender: "MALE" | "FEMALE"): SajuBasic;
}
...
describe("getSajuInfo 정확도 테스트", () => {
  testCases.forEach((testCase) => {
    it(`${testCase.name} 정확도 검증`, async () => {
      ...
      // 주요 필드별 비교
      expect(result.saju.fortuneList.storedUnse.yearSky).toBe(
        testCase.expected.saju.fortuneList.storedUnse.yearSky,
      );
