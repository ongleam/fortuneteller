  ...
  describe('Reference API 데이터와의 정확성 비교', () => {
    ...
    referenceData.forEach((testData, index) => {
      test(`케이스 ${index + 1}: ${testData.input.name} - Reference 오행 분석 비교`, async () => {
        ...
        const result = await getFiveElementsReference(birthInput);

        console.log(`\n🔍 케이스 ${index + 1}: ${testData.input.name}`);
        console.log(
          `계산: 목${result.wood} 화${result.fire} 토${result.earth} 금${result.metal} 수${result.water}`
        );
        console.log(
          `참조: 목${referenceElements.wood} 화${referenceElements.fire} 토${referenceElements.earth} 금${referenceElements.metal} 수${referenceElements.water}`
        );
