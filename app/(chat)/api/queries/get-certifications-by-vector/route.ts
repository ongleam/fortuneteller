...
export async function GET(request: NextRequest) {
  ...
  console.log(`[INFO] get-certifications-by-vector: ${query}`);
  ...
  try {
    ...
    const { data, error: rpcError } = await supabaseAdmin.rpc('get_certifications_by_vector', {
      // RPC 함수 이름 확인 필요
    ...
    if (rpcError) {
      console.error('Error calling Supabase RPC get_certifications_by_vector:', rpcError);
