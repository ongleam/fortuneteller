...
import { getPillarsTenStar } from './ten-stars';
...
export function calculateSajuInfo<T extends 'simple' | 'fetchSaju' | 'ui'>(
  ...
  try {
    ...
    // 2. 사주 팔자 계산
    const pillars = getSajuPillars(normalizedInput);
    ...
    throw new Error(
      `getSajuInfo 계산 중 오류 발생: ${error instanceof Error ? error.message : '알 수 없는 오류'}`
    );
  }
}
 ...
 */
export function calculateSajuFetchFormat(
  birthInput: BirthInput,
  targetYear?: number
): FetchSajuCompatibleOutput {
  return calculateSajuInfo(birthInput, {
 ...
 * UI 최적화 형식으로 사주 계산
 */
export function calculateSajuUiFormat(
  birthInput: BirthInput,
  targetYear?: number
): UiOptimizedSajuOutput {
  return calculateSajuInfo(birthInput, {
...
// fetchSaju 호환 형식 계산
export function getSajuInfoCompatible(
  birthInput: BirthInput,
  targetYear?: number
): FetchSajuCompatibleOutput {
  return calculateSajuFetchFormat(birthInput, targetYear);
...
// UI 최적화 형식 계산
export function getSajuInfoForUi(
  birthInput: BirthInput,
  targetYear?: number
): UiOptimizedSajuOutput {
  return calculateSajuUiFormat(birthInput, targetYear);
