...
export function calculateSajuPillars(birthInput: BirthInput): SajuPillars {
    ...
    const monthPillar = calculateMonthPillar(sajuYear, sajuMonth);
    const dayPillar = calculateDayPillar(solarDate);
    const timePillar = calculateTimePillar(dayPillar, birthInput.hour);
 ...
 */
export function calculateMonthPillar(year: number, month: number) {
 ...
 */
export function calculateTimePillar(dayPillar: { stem: string; branch: string }, hour: string) {
    ...
    // 시간을 지지로 변환
    const timeBranchKorean = CalendarConverter.getTimeToEarthlyBranch(hour);
...
export class SajuPillarsCalculator {
  static calculate(birthInput: BirthInput): SajuPillars {
    return calculateSajuPillars(birthInput);
  }
}
