 ...
 * 운세 정보 계산
 */
export function calculateFortunes(pillars: SajuPillars, targetYear?: number): FortuneInfo {
 ...
 * 현재 대운 계산
 */
export function calculateCurrentBigFortune(pillars: SajuPillars, currentYear: number): any {
 ...
 * 다음 대운 계산
 */
export function calculateNextBigFortune(pillars: SajuPillars, currentYear: number): any {
 ...
 * 대운 순행/역행 판단
 */
export function determineBigFortuneDirection(gender: string, birthYear: number): 'forward' | 'backward' {
 ...
 * 대운 시작 나이 계산
 */
export function calculateBigFortuneStartAge(pillars: SajuPillars, gender: string, birthYear: number, birthMonth: number, birthDay: number): number {
  // 복잡한 계산이 필요하므로 간단화
 ...
 * 특정 나이의 대운 계산
 */
export function getBigFortuneAtAge(pillars: SajuPillars, gender: string, birthYear: number, birthMonth: number, birthDay: number, targetAge: number): any {
 ...
 * 년운과 월운 분석
 */
export function analyzeYearlyFortune(year: number, month?: number): {
  yearFortune: any;
  monthFortune?: any;
  analysis: string;
} {
