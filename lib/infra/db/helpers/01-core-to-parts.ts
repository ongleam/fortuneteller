...
async function createNewTable() {
  ...
  for (let i = 0; i < chats.length; i += BATCH_SIZE) {
    ...
    const allMessages = await db.select().from(message).where(inArray(message.chat_id, chatIds));

    const allVotes = await db.select().from(vote).where(inArray(vote.chat_id, chatIds));
    ...
    for (const chat of chatBatch) {
      ...
      const messages = allMessages.filter((msg) => msg.chat_id === chat.id);
      const votes = allVotes.filter((v) => v.chat_id === chat.id);
            ...
            if (msg.role === 'assistant') {
              ...
              if (voteByMessage) {
                newVotesToInsert.push({
                  messageId: msg.id,
                  chatId: msg.chatId,
                  isUpvoted: voteByMessage.is_upvoted,
                });
              }
            }
          }
