...
import type { SajuPillars, TenStars } from '../../types/saju';

/**
 * 십성 계산 클래스
 */
export class SajuTenStarsCalculator {
  
  /**
   * 사주 팔자를 기준으로 십성 분석
   */
  static calculate(pillars: SajuPillars): TenStars {
    const dayStem = pillars.day.stem; // 일간이 기준
    
    return {
      yearStem: this.getTenStar(dayStem, pillars.year.stem),
      yearBranch: this.getTenStar(dayStem, this.getMainStem(pillars.year.branch)),
      monthStem: this.getTenStar(dayStem, pillars.month.stem),
      monthBranch: this.getTenStar(dayStem, this.getMainStem(pillars.month.branch)),
      dayStem: '비견', // 일간은 항상 자기 자신이므로 비견
      dayBranch: this.getTenStar(dayStem, this.getMainStem(pillars.day.branch)),
      timeStem: this.getTenStar(dayStem, pillars.time.stem),
      timeBranch: this.getTenStar(dayStem, this.getMainStem(pillars.time.branch))
    };
  }
   ...
   */
  private static getTenStar(baseStem: string, targetStem: string): string {
    ...
    // 일간이 생하는 오행 (식상)
    if (this.isGenerating(baseElement, targetElement)) {
      return targetYangYin === baseYangYin ? '식신' : '상관';
    }
    
    // 일간이 극하는 오행 (재성)
    if (this.isOvercoming(baseElement, targetElement)) {
      return targetYangYin === baseYangYin ? '편재' : '정재';
    }
    
    // 일간을 극하는 오행 (관성)
    if (this.isOvercoming(targetElement, baseElement)) {
      return targetYangYin === baseYangYin ? '편관' : '정관';
    }
    
    // 일간을 생하는 오행 (인성)
    if (this.isGenerating(targetElement, baseElement)) {
   ...
   */
  private static getMainStem(branch: string): string {
    // 지장간에서 가장 강한 천간을 반환
   ...
   * 오행 상극 관계 확인
   */
  private static isOvercoming(from: string, to: string): boolean {
