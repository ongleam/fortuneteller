...
describe('입력 정규화', () => {
  test('입력값 정규화', () => {
    ...
    expect(normalized.birthType).toBe('SOLAR');
    expect(normalized.birthYear).toBe('1995'); // 95 -> 1995로 변환
    expect(normalized.birthMonth).toBe('04');
    expect(normalized.birthDay).toBe('05');
  });
