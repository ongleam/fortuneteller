...
export class SajuAdapters {
  ...
  static toFetchSaju(data: {
    ...
    return {
      ...
      saju: {
        year: {
          stem: {
            korean: this.getKoreanStem(data.pillars.year.stem),
            chinese: data.pillars.year.stem,
            element: this.getStemElement(data.pillars.year.stem),
            yangyin: this.getStemYangYin(data.pillars.year.stem)
          },
          branch: {
            korean: this.getKoreanBranch(data.pillars.year.branch),
            chinese: data.pillars.year.branch,
            element: this.getBranchElement(data.pillars.year.branch),
            yangyin: this.getBranchYangYin(data.pillars.year.branch)
          }
        },
        month: {
          stem: {
            korean: this.getKoreanStem(data.pillars.month.stem),
            chinese: data.pillars.month.stem,
            element: this.getStemElement(data.pillars.month.stem),
            yangyin: this.getStemYangYin(data.pillars.month.stem)
          },
          branch: {
            korean: this.getKoreanBranch(data.pillars.month.branch),
            chinese: data.pillars.month.branch,
            element: this.getBranchElement(data.pillars.month.branch),
            yangyin: this.getBranchYangYin(data.pillars.month.branch)
          }
        },
        day: {
          stem: {
            korean: this.getKoreanStem(data.pillars.day.stem),
            chinese: data.pillars.day.stem,
            element: this.getStemElement(data.pillars.day.stem),
            yangyin: this.getStemYangYin(data.pillars.day.stem)
          },
          branch: {
            korean: this.getKoreanBranch(data.pillars.day.branch),
            chinese: data.pillars.day.branch,
            element: this.getBranchElement(data.pillars.day.branch),
            yangyin: this.getBranchYangYin(data.pillars.day.branch)
          }
        },
        time: {
          stem: {
            korean: this.getKoreanStem(data.pillars.time.stem),
            chinese: data.pillars.time.stem,
            element: this.getStemElement(data.pillars.time.stem),
            yangyin: this.getStemYangYin(data.pillars.time.stem)
          },
          branch: {
            korean: this.getKoreanBranch(data.pillars.time.branch),
            chinese: data.pillars.time.branch,
            element: this.getBranchElement(data.pillars.time.branch),
            yangyin: this.getBranchYangYin(data.pillars.time.branch)
          }
        }
      },
      ...
      tenStars: {
        year: {
          ...
          chinese: '',  // 임시
          meaning: this.getTenStarMeaning(data.tenStars.yearStem)
        },
        month: {
          korean: data.tenStars.monthStem,
          chinese: '',  // 임시
          meaning: this.getTenStarMeaning(data.tenStars.monthStem)
        },
        day: {
          korean: data.tenStars.dayStem,
          chinese: '',  // 임시
          meaning: this.getTenStarMeaning(data.tenStars.dayStem)
        },
        time: {
          korean: data.tenStars.timeStem,
          chinese: '',  // 임시
          meaning: this.getTenStarMeaning(data.tenStars.timeStem)
        }
      },
      ...
      elements: {
        ...
        analysis: {
          total: data.elements.wood + data.elements.fire + data.elements.earth + data.elements.metal + data.elements.water,
          strongest: this.findStrongestElement(data.elements),
          weakest: this.findWeakestElement(data.elements),
          balance: this.getElementBalance(data.elements)
        }
      },
  ...
  static toUi(data: {
    ...
    return {
      ...
      // 사주 팔자 (한글 표시)
      pillarsDisplay: {
        year: `${this.getKoreanStem(data.pillars.year.stem)}${this.getKoreanBranch(data.pillars.year.branch)}`,
        month: `${this.getKoreanStem(data.pillars.month.stem)}${this.getKoreanBranch(data.pillars.month.branch)}`,
        day: `${this.getKoreanStem(data.pillars.day.stem)}${this.getKoreanBranch(data.pillars.day.branch)}`,
        time: `${this.getKoreanStem(data.pillars.time.stem)}${this.getKoreanBranch(data.pillars.time.branch)}`
      },
      ...
      tenStarsDisplay: {
        ...
        summary: this.getTenStarsSummary(data.tenStars)
      },
      ...
      // 신살 표시
      sinsalsDisplay: data.sinsals.filter(s => s !== '').map(sinsal => ({
        name: sinsal,
        meaning: this.getSinsalMeaning(sinsal),
        type: this.getSinsalType(sinsal)
      })),
      ...
      fortuneDisplay: {
        ...
        summary: this.getFortuneSummary(data.fortune)
      }
    };
  }
  ...
  private static getTenStarMeaning(korean: string): string {
  ...
  private static findStrongestElement(elements: FiveElements): string {
  ...
  private static findWeakestElement(elements: FiveElements): string {
    const elementMap = { wood: '목', fire: '화', earth: '토', metal: '금', water: '수' };
    let minKey = 'wood';
  ...
  private static getElementBalance(elements: FiveElements): string {
    const total = elements.wood + elements.fire + elements.earth + elements.metal + elements.water;
    const avg = total / 5;
    const variance = Object.values(elements).reduce((sum, val) => sum + Math.pow(val - avg, 2), 0) / 5;
    
    return variance < 1 ? 'balanced' : 'imbalanced';
  }
  
  private static getTenStarsSummary(tenStars: TenStars): string {
  ...
  private static getSinsalMeaning(sinsal: string): string {
    const meanings: { [key: string]: string } = {
      '역마살': '이동, 변화, 활동성',
      '화개살': '예술성, 종교성, 고독함',
      '천을귀인': '귀인의 도움, 좋은 인연',
  ...
  private static getSinsalType(sinsal: string): 'good' | 'bad' | 'neutral' {
  ...
  private static getFortuneSummary(fortune: FortuneInfo): string {
